...
#if defined(UNITY_PROCEDURAL_INSTANCING_ENABLED)
	float4 color;
	color.rgb = lerp(_ColorA, _ColorB, frac(_SequenceNumbers.x * unity_InstanceID + _SequenceNumbers.y));
	color.a = lerp(_ColorA.a, _ColorB.a, frac(_SequenceNumbers.z * unity_InstanceID + _SequenceNumbers.w));
	return color;
#else
	// return _BaseColor;
	return _ColorA;
